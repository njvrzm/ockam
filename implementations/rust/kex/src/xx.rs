...
struct SymmetricState {
    ...
    static_public_key: Option<PublicKey>,
    ...
    fn prologue(&mut self) -> Result<(), VaultFailError> {
        ...
        match identity_key {
            None => {
                ...
                self.static_public_key = Some(static_public_key);
            }
            Some(ik) => {
                let public_key = vault.secret_public_key_get(&ik)?;
                self.static_public_key = Some(public_key);
            }
        };
